...
// 2023.7.10 12:12 gitae
// 게임 장르 : 로그라이크 , 디펜스

// 로그라이크와 디펜스 장르를 혼합해서 게임을 제작

// 2023.7.11 16:35 gitae
// 디펜스 장르의 특성상 오브젝트가 100개 이상인 경우가 발생하는데
// 현재의 N^2 Broadphase로는 게임의 프레임이 안정하지 못했기 때문에 
// 새로운 충돌처리 알고리즘을 도입하여야 했다 그러므로 AABBTree 알고리즘을 
// 사용해서 충돌처리를 바꾸는 중이다. 

// AABBTree에 모든 오브젝트 타입의 콜라이더를 넣는 방법을 선택하기로했다.
// 이유로는 오브젝트타입별로 트리를따로두면 확장한AABB가 수정되었을때 
// 모든 트리를 수정하고 재삽입을 해야하기 때문에 비효율적이라고 판단하였기 때문이다
// 이러한 이유로 충돌한 페어 벡터를 만들때 오브젝트 타입을 확인해서 둘의 타입이 충돌을
// 해야하면 충돌하는 식으로 수정예정이다.
// clear : CollisionManager 수정, AABBTree 보완, 오브젝트가 삭제예정 처리 

// 2023.7.12 11:40 gitae
// 현재충돌정보는 메모리 주소를 기준으로 정렬했었는데 다시 원래대로 바꿔야할꺼같다.
// 새로운 씬을 로드했을때 우연히 같은 주소를 가지는 객체가 생성되면 충돌함수가 비정상적으로
// 호출하는 문제가 발생하므로 원래대로 ColliderID값으로 정렬해야겠다.

// 2023.7.12 18:54 gitae
// 길찾기 알고리즘 
// 1. 타일맵을 생성해보자 ?
// 2. 

// 2023.7.16 21:12 gitae
// 적과 아군의 개념을 도입해야할때가 왔다
// 현재는 적과 아군의 개념이 없는 상태라서 디펜스게임으로 적당하지가 않다. 